...
void YoloObjectDetector::init() {
  ...
  modeConvPublisher_ =
      nodeHandle_.advertise<kusv_msgs::VisionMark>("mode/vision_mark", 100);
...
void* YoloObjectDetector::publishInThread() {
  ...
  if (num > 0 && num <= 100) {
    ...
    for (int i = 0; i < numClasses_; i++) {
      if (rosBoxCounter_[i] > 0) {
        ...
        for (int j = 0; j < rosBoxCounter_[i]; j++) {
          ...
          m_visionSign.height = BoundingBox_height;
          m_visionSign.width = BoundingBox_width;
          m_visionSign.sign = 1;
          ROS_INFO("Detected");
          modeConvPublisher_.publish(m_visionSign);
        }
      }
    }
    ...
    m_visionSign.height = 0.0;
    m_visionSign.width = 0.0;
    m_visionSign.sign = 0;
    ROS_INFO("Non Detected");
    modeConvPublisher_.publish(m_visionSign);
  }
